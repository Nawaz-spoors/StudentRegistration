...
<form action="logout">
<button class="bg-warning" style="align:left;" type="submit">logout</button>
</form>
<div align="center">
<h1>School Students</h1>
<form action="addStudent" > 
	<button  class="bg-dark" style="color:white;"><b>Add Student</b></button>
</form>
	<table>
	<thead>
		<tr>
	...
	<tr >
		...
		<%-- <td>${item.mobile}</td>
		<td>${item.dob}</td> --%>
		<td><a href="find?sid=${item.id}">edit</a></td>
		<td><a href="delete?sid=${item.id}" onclick="if(!confirm('DELETE STUDENT???')) return false">delete</a></td>
		<td id="s${item.id}"><button>info</button></td>
		<!-- <td></td>
		<td></td> -->
	</tr>
